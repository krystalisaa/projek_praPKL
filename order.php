...
if ($status) {
    $sql .= " AND pesanan.status = '" . mysqli_real_escape_string($koneksi, $status) . "'";
}

$sql .= " ORDER BY pesanan.tgl_pesan DESC";
...
<head>
  ...
  <style>
    * {
    ...
    .detail-item:hover .quantity-badge,
    .detail-item:hover .total-price,
    .detail-item:hover .payment-badge {
      transform: scale(1.05);
    }
...
<body>
  ...
  <div class="container">
    <!-- Navigation Tabs -->
    <div class="order-nav">
      <a href="order.php" class="<?= !isset($_GET['status']) ? 'active' : '' ?>">
        <i class="bi bi-list-ul me-2"></i>Semua
      </a>
      <a href="produk-pending.php" class="<?= (isset($_GET['status']) && $_GET['status'] == 'Pending') ? 'active' : '' ?>">
        <i class="bi bi-clock-history me-2"></i>Pending
      </a>
      <a href="produk-delivery.php" class="<?= (isset($_GET['status']) && $_GET['status'] == 'Delivery') ? 'active' : '' ?>">
        <i class="bi bi-truck me-2"></i></i>Delivery
      </a>
      <a href="produk-completed.php" class="<?= (isset($_GET['status']) && $_GET['status'] == 'Completed') ? 'active' : '' ?>">
        <i class="bi bi-check-circle me-2"></i>Completed
      </a>
      <a href="produk-canceled.php" class="<?= (isset($_GET['status']) && $_GET['status'] == 'Canceled') ? 'active' : '' ?>">
        <i class="bi bi-x-circle me-2"></i></i>Canceled
      </a>
    </div>
    ...
    ?>
      <div class="order-card">
        ...
        <div class="status-badge <?= $statusClass ?>">
          <i class="bi bi-<?= $statusIcon ?> status-icon"></i><?= htmlspecialchars($data['status']); ?>
        </div>
        ...
        <div class="order-details">
          ...
          <div class="detail-item">
            ...
            <div class="payment-badge"><?= htmlspecialchars($data['pembayaran']); ?></div>
          </div>
        </div>
        ...
        <div class="customer-info">
          ...
          <div class="customer-details">
            ...
            <?php if (!empty($data['nama_kota'])): ?>
            <div>
              <i class="bi bi-building"></i>
              <?= htmlspecialchars($data['nama_kota']); ?>
            </div>
            <?php endif; ?>
          </div>
        </div>
